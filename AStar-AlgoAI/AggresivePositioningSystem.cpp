...
#include <iostream>
	...
	AggresivePositioningSystem::AggresivePositioningSystem()
	{

	}

	AggresivePositioningSystem::AggresivePositioningSystem(Agent* owner)
	{
		m_pOwner = owner;
	...
	void AggresivePositioningSystem::update()
	{
		...
		if(isTargetPlayerPresent()){
			if( m_pOwner->m_Agent_Action == NOTHING  && isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				m_pOwner->m_Agent_Action = SHOOTING;
			}else if(m_pOwner->m_Agent_Action == NOTHING && !isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				FindNewPostionForLS(); // 1:Set Action to Moving 2:Find new Move Location 3:Find Path to new move
			}
			else if( m_pOwner->m_Agent_Action == SHOOTING  && isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				return ;
			}
			else if (m_pOwner->m_Agent_Action == SHOOTING && !isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			...
			else if (m_pOwner->m_Agent_Action == MOVING && !isClosest(*m_pOwner->m_pCurrent_Move_target, *m_pOwner->m_pCurrent_target))
			{
				FindNewPostionForLS(); // 1:Set Action to Moving 2:Find new Move Location 3:Find Path to new move
			}
		}
	...
	void AggresivePositioningSystem::FindNewPostionForLS() // 1:Set Action to Moving 2:Find new Move Location 3:Find Path to new move
	{
		//1. Find the New cube to move which has Line of sight.
		ds::Node* pathNodes = aggresivePositionFinding.getClosestPosition(*m_pOwner->m_pCurrent_location,  *m_pOwner->m_pCurrent_target);
		Node node = *pathNodes;
	...
	bool AggresivePositioningSystem::isTargetPlayerPresent()
	{
	...
	bool AggresivePositioningSystem::isClosest(ds::CubeGeomerty startLocation, ds::CubeGeomerty targetLocation)
	{
		bool result = false;
	...
	bool AggresivePositioningSystem::isLineOfSightPresent(ds::CubeGeomerty startLocation, ds::CubeGeomerty targetLocation)
	{
		bool result = true; 
		//if(isTargetPlayerPresent)
		{
