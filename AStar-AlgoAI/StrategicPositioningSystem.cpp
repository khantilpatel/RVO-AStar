...
#include <iostream>
	...
	StrategicPositioningSystem::StrategicPositioningSystem()
	{

	}

	StrategicPositioningSystem::StrategicPositioningSystem(Agent* owner)
	{
		m_pOwner = owner;
	...
	void StrategicPositioningSystem::update()
	{
		...
		if(isTargetPlayerPresent()){
			if( m_pOwner->m_Agent_Action == NOTHING  && isLineOfSightPresent(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))//isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				m_pOwner->m_Agent_Action = SHOOTING;
			}else if(m_pOwner->m_Agent_Action == NOTHING && !isLineOfSightPresent(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))// !isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				FindNewPostionForLS(); // 1:Set Action to Moving 2:Find new Move Location 3:Find Path to new move
			}
			else if( m_pOwner->m_Agent_Action == SHOOTING && isLineOfSightPresent(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target)) // && isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			{
				return ;
			}
			else if (m_pOwner->m_Agent_Action == SHOOTING && !isLineOfSightPresent(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))// !isClosest(*m_pOwner->m_pCurrent_location, *m_pOwner->m_pCurrent_target))
			...
			else if (m_pOwner->m_Agent_Action == MOVING && !isLineOfSightPresent(*m_pOwner->m_pCurrent_Move_target, *m_pOwner->m_pCurrent_target)) //!isClosest(*m_pOwner->m_pCurrent_Move_target, *m_pOwner->m_pCurrent_target))
			{
				FindNewPostionForLS(); // 1:Set Action to Moving 2:Find new Move Location 3:Find Path to new move
			}
		}
	...
	void StrategicPositioningSystem::pathFindingAStar()
	{
		// 1. find path
		if(m_pOwner->m_Move_path_nodes.size() == 0){
			

		/*	m_pOwner->m_Move_path_nodes.push_back(new ds::CubeGeomerty(m_pOwner->m_pCurrent_Move_target_value.location));

			m_pOwner->m_Move_path_nodes.push_back(new ds::CubeGeomerty(m_pOwner->m_pCurrent_Move_target_value.location));

			m_pOwner->m_Move_path_nodes.push_back(new ds::CubeGeomerty(m_pOwner->m_pCurrent_location_value.location));*/
			ds::Node*  pathNodes = pathfindingAStar.A_Star(m_pOwner->m_pCurrent_location_value, m_pOwner->m_pCurrent_Move_target_value);
	...
	bool StrategicPositioningSystem::isTargetPlayerPresent()
	{
	...
	bool StrategicPositioningSystem::isClosest(ds::CubeGeomerty startLocation, ds::CubeGeomerty targetLocation)
	{
		bool result = false;
		int x_temp = abs((int)startLocation.location.x - (int)targetLocation.location.x);
		int y_temp = abs((int)startLocation.location.y - (int)targetLocation.location.y);
		int H = (x_temp+y_temp)*10;
		if(H == 10 || H == 20)
		{
			result = true;
		}
	...
	bool StrategicPositioningSystem::isLineOfSightPresent(ds::CubeGeomerty startLocation, ds::CubeGeomerty targetLocation)
	{
		bool result = true; 
		//if(isTargetPlayerPresent)
		{
